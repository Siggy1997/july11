 ...
 * 			= 컨트롤 기능을 하는 인스턴스
 * 			= 사용자의 요청이 들어오면 해당 URL에 맞는 페이지에 데이터를 보내줍니다
 * 
 * 
 * Service = 프로그램로직
 * 
 * DTO/VO
 * DAO = 데이터베이스와 연결되어서 작업합니다 -> mybatis에서 그냥 함 
 * 
 * jsp = view 보여지는 화면
 * 
 *  사용자 -> 컨트롤러 -> 서비스 -> DAO -> mybatis -> DB   --->  반대로 다시 돌아감
 *  DB -> mybatis -> DAO -> 서비스 -> 컨트롤러 -> jsp+db -> 사용지
 * 
 * 연결 3가지 = DI의존 주입
 ...
 *  
 *  1. DI(Dependancy Injection) 의존 주입
 ...
 *  		메서드가 실행되기전, 실행된 직후 실행시점에 따라 따로 기능을 적용시키는것
 *  
 *  
 *  
 */
...
public class BoardController {
	...
	@RequestMapping(value = "/main") //url 도메인
